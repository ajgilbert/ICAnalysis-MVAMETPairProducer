#ifndef RecoMET_METPUSubtraction_CommonMVAMETPairProducer_h
#define RecoMET_METPUSubtraction_CommonMVAMETPairProducer_h

/** \class CommonMVAMETPairProducer
 *  *
...
namespace reco
{
  class CommonMVAMETPairProducer : public edm::stream::EDProducer<>
  {
    public:

      CommonMVAMETPairProducer(const edm::ParameterSet&); 
      ~CommonMVAMETPairProducer();
    ...
    private:
      ...
      // auxiliary functions
      std::pair<std::vector<reco::PUSubMETCandInfo>,std::size_t> computeLeptonInfo(const std::vector<edm::EDGetTokenT<reco::CandidateView > >& srcLeptons_,
	  const reco::CandidateView& pfCandidates,
	  const reco::Vertex* hardScatterVertex, 
	  int& lId, bool& lHasPhotons, edm::Event & iEvent);

      std::pair<std::vector<reco::PUSubMETCandInfo>,std::size_t> computeLeptonInfo(const std::vector<edm::RefToBase<const reco::Candidate>>& srcLeptons_, const reco::CandidateView& pfCandidates_view,
	  const reco::Vertex* hardScatterVertex,
