#include "ICAnalysis/MVAMETPairProducer/plugins/CommonMVAMETPairProducer.hh"
...
CommonMVAMETPairProducer::~CommonMVAMETPairProducer(){}

void CommonMVAMETPairProducer::produce(edm::Event& evt, const edm::EventSetup& es) 
{ 
  ...
  if ( minNumLeptons_ > 0 ) {
    ...
    if ( !(numLeptons >= minNumLeptons_) ) {
      LogDebug( "produce" )
	<< "<CommonMVAMETPairProducer::produce>:" << std::endl
  ...
  for (const auto& permutation : permutations) {
	...
	leptonPtrs.emplace_back(leptons->ptrAt(pos));
      ...
      leptonInfo = std::move(computeLeptonInfo(perm_leptons, *pfCandidates_view,hardScatterVertex, lId, lHasPhotons).first);
      id_part = std::move(computeLeptonInfo(perm_leptons, *pfCandidates_view,hardScatterVertex, lId, lHasPhotons).second);
      
    }
    else {
      std::vector<reco::PUSubMETCandInfo> leptonInfo = computeLeptonInfo(srcLeptons_, *pfCandidates_view, hardScatterVertex, lId, lHasPhotons, evt).first;
      id_part = computeLeptonInfo(srcLeptons_, *pfCandidates_view, hardScatterVertex, lId, lHasPhotons, evt).second;
    }
...
CommonMVAMETPairProducer::getPermutations(std::vector<size_t> lengths, std::vector<size_t>& current, std::vector<std::vector<size_t> >& result)
{ 
...
std::vector<std::vector<size_t> > CommonMVAMETPairProducer::getPermutations(const edm::Event& evt) {
...
std::pair<std::vector<reco::PUSubMETCandInfo>,std::size_t>
CommonMVAMETPairProducer::computeLeptonInfo(const std::vector<edm::RefToBase<const reco::Candidate>>& srcLeptons,
  ...
  return std::make_pair(leptonInfo,id);
}


std::pair<std::vector<reco::PUSubMETCandInfo>,std::size_t>
CommonMVAMETPairProducer::computeLeptonInfo(const std::vector<edm::EDGetTokenT<reco::CandidateView > >& srcLeptons_,
    const reco::CandidateView& pfCandidates_view,
    const reco::Vertex* hardScatterVertex,
...
CommonMVAMETPairProducer::computeJetInfo(const reco::PFJetCollection& uncorrJets, 
    ...
    std::vector<reco::PUSubMETCandInfo> &iLeptons,std::vector<reco::PUSubMETCandInfo> &iCands)
{
  ...
  std::vector<reco::PUSubMETCandInfo> CommonMVAMETPairProducer::computePFCandidateInfo(const reco::CandidateView& pfCandidates,
      const reco::Vertex* hardScatterVertex)
  {
    std::vector<reco::PUSubMETCandInfo> retVal;
  ...
  std::vector<reco::Vertex::Point> CommonMVAMETPairProducer::computeVertexInfo(const reco::VertexCollection& vertices)
  {
  ...
  double CommonMVAMETPairProducer::chargedEnFrac(const reco::Candidate *iCand,
  ...
  bool CommonMVAMETPairProducer::istau(const reco::Candidate *iCand) { 
    if(iCand->isMuon())     return false;
    if(iCand->isElectron()) return false;
    if(iCand->isPhoton())   return false;
    return true;
  }
  bool CommonMVAMETPairProducer::passPFLooseId(const PFJet *iJet) { 
  ...
  double CommonMVAMETPairProducer::chargedFracInCone(const reco::Candidate *iCand,
...
#include "FWCore/Framework/interface/MakerMacros.h"
  ...
  DEFINE_FWK_MODULE(CommonMVAMETPairProducer);
